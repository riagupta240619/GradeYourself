...
export function UploadResultsModal({ isOpen, onClose, onSuccess, manualMode = false }: UploadResultsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/35 dark:bg-black/65 backdrop-blur-md p-4 overflow-y-auto">
      <div className="relative w-full max-w-5xl rounded-2xl bg-white border border-slate-200 text-slate-900 shadow-2xl overflow-hidden flex flex-col max-h-[92vh] dark:bg-slate-900 dark:border-slate-800 dark:text-slate-100">
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/90 dark:border-slate-800 dark:bg-slate-950/60">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          {errorMsg && (
            <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-700 dark:text-rose-300 flex items-start gap-3 text-sm">
              <AlertCircle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-semibold">Processing Notice</p>
                <p className="text-slate-700 dark:text-slate-300">{errorMsg}</p>
              </div>
            </div>
          )}

          {successMsg && (
            <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300 flex items-center gap-3 text-sm">
          ...
          {pipelineStep === "idle" && (
            <div className="space-y-6">
              <div
                ...
                className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all duration-200 ${
                  dragOver
                    ...
                    : "border-slate-200 hover:border-indigo-500/50 bg-slate-50 hover:bg-indigo-50/60 dark:border-slate-800 dark:bg-slate-950/40 dark:hover:bg-slate-950/70"
                }`}
              >
                <input
                  type="file"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm dark:bg-slate-950/40 dark:border-slate-800/80">
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between dark:border-slate-800 dark:bg-slate-950/80">
          {pipelineStep === "review" ? (
            <Button
              variant="outline"
              onClick={() => {
