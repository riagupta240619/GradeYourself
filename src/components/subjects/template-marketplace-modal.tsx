    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 sm:p-6 backdrop-blur-md animate-in fade-in duration-200">
      <div className="flex h-[90vh] w-full max-w-5xl flex-col rounded-3xl border border-white/10 bg-zinc-950/95 shadow-2xl overflow-hidden backdrop-blur-2xl">
        ...
        <div className="flex items-center justify-between border-b border-white/10 p-6 bg-zinc-900/60">
          ...
          <button
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-zinc-900 text-zinc-400 hover:bg-zinc-800 hover:text-white transition"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-white/10 px-6 py-4 bg-zinc-950">
          <div className="flex items-center gap-1.5 rounded-xl bg-zinc-900/80 p-1 border border-white/10 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                category === "all" ? "bg-purple-600 text-white shadow-sm" : "text-zinc-400 hover:text-white"
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                category === "verified" ? "bg-purple-600 text-white shadow-sm" : "text-zinc-400 hover:text-white"
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                category === "custom" ? "bg-purple-600 text-white shadow-sm" : "text-zinc-400 hover:text-white"
              }`}
            >
          ...
          <div className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-zinc-900/90 px-3.5 py-2 text-xs text-zinc-400 w-full sm:w-72">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="py-20 text-center flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-zinc-400">Loading evaluation scheme repository...</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="py-20 text-center flex flex-col items-center justify-center">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((t) => {
                ...
                return (
                  <Card key={t.id || (t as any)._id} className="border border-white/10 bg-zinc-900/90 hover:border-purple-500/40 transition-all flex flex-col justify-between shadow-xl">
      ...
      {previewTemplate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 backdrop-blur-md">
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-zinc-950 p-6 flex flex-col gap-4 shadow-2xl">
            ...
            <div className="flex flex-col gap-3 max-h-96 overflow-y-auto">
              {normalizeScheme(previewTemplate).components.map((comp) => (
                <div key={comp.id} className="rounded-xl border border-white/10 bg-zinc-900 p-3.5 flex flex-col gap-2 text-xs">
