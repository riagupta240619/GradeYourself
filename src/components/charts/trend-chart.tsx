...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-zinc-900/95 p-3.5 text-xs shadow-2xl backdrop-blur-md min-w-[200px] flex flex-col gap-2">
        ...
        {isProj ? (
          <div className="flex flex-col gap-1.5 font-mono">
            ...
            <div className="flex items-center justify-between gap-3 text-purple-600 dark:text-purple-300">
            ...
            <p className="text-[10px] text-zinc-400 italic pt-1.5 border-t border-slate-200 dark:border-white/10 font-sans">
...
export function TrendChart({ data, mode = "official" }: { data: TrendChartPoint[]; mode?: "official" | "predicted" }) {
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <div className="h-60 w-full flex items-center justify-center text-xs text-zinc-500 italic border border-dashed border-slate-200 dark:border-white/10 rounded-xl my-2">
  ...
  return (
    <div className="flex flex-col gap-3 w-full pt-2">
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-semibold px-2 py-1 bg-slate-100 dark:bg-zinc-950/40 rounded-xl border border-slate-200 dark:border-white/5">
        <button
          onClick={() => toggleSeries("officialCgpa")}
          className={`flex items-center gap-1.5 transition-all ${visibleSeries.officialCgpa ? "text-purple-700 dark:text-purple-300 opacity-100" : "text-slate-400 dark:text-zinc-500 opacity-40 line-through"}`}
        >
          <span className="h-2.5 w-4 rounded-full bg-purple-600 dark:bg-purple-500" />
        ...
        {showPredictions && (
          <button
            ...
            className={`flex items-center gap-1.5 transition-all ${visibleSeries.projectedCgpa ? "text-purple-700 dark:text-purple-300 opacity-100" : "text-slate-400 dark:text-zinc-500 opacity-40 line-through"}`}
          >
            <span className="h-2.5 w-4 rounded-full bg-purple-500 border border-dashed border-purple-300 dark:bg-purple-400 dark:border-purple-200" />
        ...
        <button
          ...
          className={`flex items-center gap-1.5 transition-all ${visibleSeries.officialSgpa ? "text-blue-700 dark:text-blue-300 opacity-100" : "text-slate-400 dark:text-zinc-500 opacity-40 line-through"}`}
        >
          <span className="h-2.5 w-4 rounded-full bg-blue-600 dark:bg-blue-500" />
        ...
        {showPredictions && (
          <button
            ...
            className={`flex items-center gap-1.5 transition-all ${visibleSeries.projectedSgpa ? "text-blue-700 dark:text-blue-300 opacity-100" : "text-slate-400 dark:text-zinc-500 opacity-40 line-through"}`}
          >
            <span className="h-2.5 w-4 rounded-full bg-blue-500 border border-dashed border-blue-300 dark:bg-blue-400 dark:border-blue-200" />
