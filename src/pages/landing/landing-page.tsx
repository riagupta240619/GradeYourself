    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-slate-50 text-slate-900 dark:bg-[#09090b] dark:text-white selection:bg-purple-500/30 selection:text-white">
      ...
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-slate-200 bg-white/80 dark:border-white/10 dark:bg-zinc-950/80 px-6 sm:px-12 backdrop-blur-2xl">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={toggleTheme}
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white/80 dark:border-white/10 dark:bg-zinc-900 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors shadow-sm"
            aria-label="Toggle Theme"
          >
            {theme === "dark" ? <Sun size={16} className="text-amber-400" /> : <Moon size={16} className="text-purple-600" />}
          </button>
          <Link to="/login">
            <Button variant="ghost" size="sm" className="text-slate-700 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-white">
              Sign in
            </Button>
          </Link>
          <Link to="/register">
      ...
      <section className="relative mx-auto flex max-w-5xl flex-col items-center gap-8 px-6 pt-20 pb-16 text-center">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 rounded-full border border-purple-300 bg-purple-50 dark:border-purple-500/30 dark:bg-purple-500/10 px-4 py-1.5 text-xs font-semibold text-purple-700 dark:text-purple-300 shadow-sm dark:shadow-[0_0_20px_rgba(124,58,237,0.2)] backdrop-blur-md"
        >
          <Sparkles size={14} className="text-purple-500 dark:text-purple-400 animate-pulse" />
          <span>Next-Gen Academic Analytics & Grading OS</span>
          <span className="rounded-full bg-purple-200 dark:bg-purple-500/30 px-2 py-0.5 text-[10px] text-purple-700 dark:text-purple-200">v2.4</span>
        </motion.div>
        ...
        <motion.h1
          ...
          className="text-4xl font-extrabold tracking-tight sm:text-6xl lg:text-7xl leading-tight text-slate-900 dark:text-white"
        >
        ...
        >
          ...
          <div className="mb-4 flex items-center justify-between border-b border-slate-200 dark:border-white/10 pb-3 text-xs text-slate-500 dark:text-zinc-400">
            ...
            <div className="rounded-lg bg-slate-100 dark:bg-zinc-800/80 px-4 py-1 text-[11px] font-mono text-slate-600 dark:text-zinc-400">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-left">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-zinc-950/60 p-4">
              <span className="text-xs uppercase text-slate-500 dark:text-zinc-500 font-semibold">Target Goal</span>
              <div className="mt-1 text-3xl font-bold font-tabular text-purple-600 dark:text-purple-400">9.50</div>
              <span className="text-[11px] text-slate-500 dark:text-zinc-400">Semester 4 Benchmark</span>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-zinc-950/60 p-4">
              <span className="text-xs uppercase text-slate-500 dark:text-zinc-500 font-semibold">Target Feasibility</span>
              <div className="mt-1 flex items-center gap-2">
                <span className="rounded-full bg-emerald-100 dark:bg-emerald-500/20 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/30">
                  Highly Achievable ✓
                </span>
              </div>
            </div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f, i) => (
            ...
            >
              <div className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl bg-purple-100 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-500/20 group-hover:bg-purple-600 group-hover:text-white transition-colors duration-300">
                <f.icon size={22} />
              </div>
              <h3 className="mb-1 text-base font-semibold text-slate-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-300 transition-colors">
                {f.title}
              </h3>
